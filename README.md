...
A extensão pede apenas a URL base nas configurações, envia `FormData` com o campo `frase` para `/sign-process-pose`, lê o `task_id` retornado e consulta imediatamente `/task-status/<task_id>` e repete a cada 2 segundos por até 1 minuto, até receber `file_url`.
